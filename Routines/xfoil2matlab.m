...
%           from the TE-bottm passed the LE to the TE bottom)
...
if (fid<=0)
    ...
    for ii = 1:length(alpha)
        ...
        if (fid<=0)
            ...
            D = textscan(fid,'%f%f%f%f%f%f%f%f%f%f%f%f','Delimiter',' ','MultipleDelimsAsOne',true,'CollectOutput',1,'HeaderLines',1);
            fclose(fid);
    ...
    if exist([wd filesep file_pwrt],'file') == 0
        error([mfilename ':io'],'Unable to read xfoil polar file %s',file_pwrt);
    else
        % Header
        % Calculated polar for: NACA 0012
        %[wd filesep file_pwrt]
        P = fileread([[wd filesep file_pwrt]]);
        Prow = strsplit(P,'\n');
        
        pol.name = strsplit(Prow{4},':');
        pol.name = strtrim(pol.name{2});
        
        %     % xtrf =   1.000 (top)        1.000 (bottom)
        temp_str = strsplit(Prow{8},' ');
        pol.xtrf_top = str2double(temp_str{4});
        pol.xtrf_bot = str2double(temp_str{6});
        clear temp_str
        
        %     % Mach =   0.000     Re =     1.000 e 6     Ncrit =  12.000
        temp_str = strsplit(Prow{9},' ');
        pol.Mach = str2double(temp_str{4});
        pol.Re   = str2double(temp_str{7})*10^str2double(temp_str{9});
        pol.Ncrit = str2double(temp_str{12});
        clear temp_str
        
        MAT = nan(size(Prow,2)-13,9);
        
        for i = 13:size(Prow,2)-1
            
            temp = strsplit(Prow{i},' ');
            for j = 2:size(temp,2)
                MAT(i-12,j-1) = str2double(temp{j});
            end
