...
ffoil = xfoil_distr(alpha,'NACA0012','coeff',xfoil_cmd,100);
fprintf('Xfoil inv; CL = %01.5f ; CD = %01.6f;\n',ffoil(end,1),ffoil(end,2))

for i = 1:size(n,2) % n pannelli 
    
tstart = tic;    
[xp,yp] = NACA_generator(naca,n(i),'cos',0);
...
f(i,:) = pdistr(alpha,U_mag,xp,yp,'coeff');
t(i) = toc(tstart);

if i == 1
fprintf('%03d/%03d; n = %03.1d ; CL = %01.5f; CD = %01.6f; t = %03.1f \n',...
    i,size(n,2),n(i),f(end,1),f(end,2),t(end));
else
fprintf('%03d/%03d; n = %03.1d ; CL = %01.5f (errCL-1 = %01.5f); CD = %01.6f; t = %03.1f \n',...
    i,size(n,2),n(i),f(end,1),f(end,1)-f(end-1,1),f(end,2),t(end));
end
end
figure; plot(n,f(:,1),'g'); grid on
figure; plot(n,f(:,2),'g'); grid on
...
[pol,foil] = xfoil2matlab('NACA0012',alpha_v,0,0,1000,xfoil_cmd);
